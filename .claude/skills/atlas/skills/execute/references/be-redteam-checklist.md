...
- [ ] **BATCH-4** — 스케줄러에 타임존이 명시되어 있는가
  → line_ref: 스케줄 정의 위치
- [ ] **BATCH-5** — 배치 출력(Writer/Sink) 단계가 빈 구현이 아닌 실제 저장 로직을 포함하는가
  → 빈 람다/no-op 구현 + ORM dirty checking 의존은 flush 타이밍에 따라 데이터 유실 가능
  → line_ref: Writer/Sink 구현 위치
...
- [ ] **INCOMPLETE-2** — Task AC의 MUST 항목 중 API endpoint 관련 항목이 라우트/핸들러에 모두 매핑되었는가
  → AC가 N개 endpoint를 요구하면 N개 핸들러가 존재해야 한다
  → line_ref: AC 항목 위치 + 핸들러 메서드/함수 위치 (누락 시 FAIL + 구현 범위 명시)
