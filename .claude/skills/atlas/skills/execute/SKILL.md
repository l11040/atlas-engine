...
**각 Task를 별도 Agent로 실행하여 컨텍스트 압축 문제를 방지한다.**

```
메인 에이전트 (오케스트레이터)           서브에이전트 (Task 단위)
┌──────────────────────┐          ┌──────────────────────┐
│ Task 목록 관리        │          │ 독립 컨텍스트          │
│ 의존성 순서 결정       │──Agent──→│ 코드 생성              │
│ 성공/실패 집계        │          │ convention-check       │
│ 다음 Task 디스패치    │          │ 레드팀                 │
│                      │←─결과────│ validate.sh            │
│ (컨텍스트 짧게 유지)   │          │ 커밋                   │
└──────────────────────┘          └──────────────────────┘
...
**왜 서브에이전트인가:**
- 메인 에이전트의 컨텍스트가 길어지면 자동 압축이 발생하고, 압축 시 convention-check 같은 단계를 건너뛰는 문제가 실제로 발생했다
- 서브에이전트는 단일 Task만 처리하므로 컨텍스트 압축이 거의 발생하지 않는다
- completion-gate가 증거를 요구하므로, 서브에이전트가 단계를 건너뛰어도 차단된다

#### Agent 호출 방법

각 Task에 대해 Agent tool을 호출한다:

```
Agent(prompt: """
아래 Task를 실행하라. 모든 단계를 빠짐없이 수행해야 한다.
...
1. Hooks 환경변수 설정
2. 코드 생성
3. convention-check (증거 필수: convention-check.json)
4. pre-build 검증
5. 레드팀 (레이어별 병렬)
6. 피드백 반영
7. validate.sh 실행 (증거 필수: validate.json)
8. 성공 시 커밋 (complete_task)

## convention-check 절차
convention-registry.yaml을 읽고, Task의 파일 패턴에 맞는 스킬을 각각 실행한다.
결과를 record-convention-evidence.sh로 기록한다.
이 증거가 없으면 completion-gate가 차단한다.

## 완료 조건
- convention-check.json PASS
- validate.json PASS
- redteam-summary.json 존재
- complete_task 호출 완료
""")
```
...
서브에이전트 반환 후:
1. `evidence/execute/task-{id}/` 하위 증거 파일 확인
2. Task status 확인 (`get_task_status`)
3. 성공 → 다음 Task로
4. 실패 → 실패 이유 기록 + 다음 Task로 (또는 중단)

**이 구조에서 메인 에이전트의 컨텍스트는 Task 목록 + 결과 요약만 유지하므로 압축이 발생하지 않는다.**

---

**각 서브에이전트 내부의 Task 실행 절차:**
...
- `completion-gate.sh`: Stop 시 validate + convention-check + 레드팀 증거 확인
