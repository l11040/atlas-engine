...
Run 결정:
1. `resolve_run(TICKET_KEY)` — 기존 활성 run이 있으면 이어서 진행
2. 없으면 `create_run(TICKET_KEY)` — 새 run 생성
3. `RUN_DIR`을 해당 run의 절대 경로로 설정

### 2. Learn — 컨벤션 학습

- `conventions.json`이 이미 존재하면 스킵 + 스킵 증거 기록:
  ```bash
...
- `evidence/analyze/done.json`이 존재하고 `status=done`이면 스킵
- 없으면 `skills/analyze/SKILL.md`를 읽고 실행

### 4. Execute — 코드 생성 + 검증 + 커밋

- `skills/execute/SKILL.md`를 읽고 실행
- 각 Task에 대해: 코드 생성 → pre-build → BE 레드팀(병렬) → 피드백 반영 → validate.sh → 커밋
