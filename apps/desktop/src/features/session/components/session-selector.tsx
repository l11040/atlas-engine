...
export function SessionSelector({ sessions, selectedId, onSelect, loading }: SessionSelectorProps) {
  if (loading && sessions.length === 0) {
    return <span className="text-xs text-text-soft">로딩 중...</span>;
  }
  ...
  return (
    <Select value={selectedId ?? undefined} onValueChange={onSelect}>
      <SelectTrigger className="h-7 w-64 border-border-subtle bg-surface-subtle text-xs text-text-strong">
        ...
        {sessions.map((s) => (
          <SelectItem key={s.sessionId} value={s.sessionId} className="text-xs">
            ...
            <span className="ml-2 text-text-soft">
              {fmtSessionTime(s.startedAt)} · A:{s.agentCount} S:{s.skillCount}
            </span>
          </SelectItem>
        ))}
      </SelectContent>
