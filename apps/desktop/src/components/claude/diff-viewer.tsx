...
const STATUS_LABELS: Record<GitDiffFileEntry["status"], { text: string; className: string }> = {
  added: { text: "추가", className: "border-transparent bg-diff-addition-bg text-diff-addition-text hover:bg-diff-addition-bg" },
  modified: { text: "수정", className: "border-transparent bg-diff-modified-bg text-diff-modified-text hover:bg-diff-modified-bg" },
  deleted: { text: "삭제", className: "border-transparent bg-diff-deletion-bg text-diff-deletion-text hover:bg-diff-deletion-bg" },
  renamed: { text: "이름변경", className: "border-transparent bg-diff-hunk-bg text-diff-hunk-text hover:bg-diff-hunk-bg" }
};
  ...
  return (
    <Collapsible className="border-b border-border-subtle last:border-b-0">
      <CollapsibleTrigger className="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-surface-subtle transition-colors [&[data-state=open]>svg.chevron]:rotate-90">
        ...
        <Badge className={cn("shrink-0 text-2xs py-0", statusMeta.className)}>
          {statusMeta.text}
        </Badge>
        ...
        <div className="overflow-x-auto bg-surface-subtle px-1 pb-2 font-mono text-2xs leading-relaxed">
          {file.hunks.map((hunk, hi) => (
            <div key={hi} className="mt-1">
              <div className="rounded bg-diff-hunk-bg px-2 py-0.5 text-diff-hunk-text">
                {hunk.header}
              </div>
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-xs border border-border-subtle bg-surface-base">
      <div className="flex items-center gap-2 border-b border-border-subtle bg-surface-subtle px-3 py-2">
        ...
        {diff.error && (
          <span className="ml-auto text-2xs text-status-danger">{diff.error}</span>
        )}
      </div>
